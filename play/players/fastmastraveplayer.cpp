...
bool FASTMASTRAVEPlayer::adjustArrays(std::size_t branch)
{
	if(!MASTRAVEPlayer::adjustArrays(branch))
		return false;
...
void FASTMASTRAVEPlayer::newGame(cadiaplayer::play::GameTheory& theory)
{
	MASTRAVEPlayer::newGame(theory);
	newGameTilingEndSegment(theory, m_roleindex);
}

void FASTMASTRAVEPlayer::newSearch(cadiaplayer::play::GameTheory& theory)
{
	MASTRAVEPlayer::newSearch(theory);
	newSearchTilingEndSegment(theory);
}

void FASTMASTRAVEPlayer::atTerminal(cadiaplayer::play::GameTheory& theory, std::vector<double>& qValues)
{
	MASTRAVEPlayer::atTerminal(theory, qValues);
	atTerminalTilingEndSegment(theory, qValues);
}

void FASTMASTRAVEPlayer::makeAction(cadiaplayer::play::GameTheory& theory, cadiaplayer::play::PlayMoves& pm, std::vector<double>& rewards)
{
	MASTRAVEPlayer::makeAction(theory, pm, rewards);
	makeActionTilingEndSegment(theory);
}
cadiaplayer::play::utils::MTNode* FASTMASTRAVEPlayer::makeAction(cadiaplayer::play::GameTheory& theory, cadiaplayer::play::utils::MTStateID sid, cadiaplayer::play::utils::Depth depth, cadiaplayer::play::PlayMoves& moves)
{
	MTNode* node = MASTRAVEPlayer::makeAction(theory, sid, depth, moves);
	makeActionTilingEndSegment(theory);
	return node;
}

std::string FASTMASTRAVEPlayer::getLastPlayInfo(cadiaplayer::play::GameTheory& theory) 
{
	std::stringstream ss;
	ss << MASTRAVEPlayer::getLastPlayInfo(theory);
	ss << std::endl;
	ss << getLastPlayInfoTilingEndSegment(theory);
	return ss.str();
}

size_t FASTMASTRAVEPlayer::selectAction(cadiaplayer::play::GameTheory& theory, cadiaplayer::play::RoleMoves& moves)
{
	if(moves.size() == 1)
		return 0;
