...
transform_formula(true, true) :- !.
transform_formula((F, G), (Fp, Gp)) :- !,
	transform_formula(F, Fp),
	transform_formula(G, Gp).
transform_formula((F; G), (Fp; Gp)) :- !,
	transform_formula(F, Fp),
	transform_formula(G, Gp).
transform_formula(not F, ~ Fp) :- !,
	transform_formula(F, Fp).
transform_formula(P, Pp) :-
	rename_predicate(P,Pp).
...
load_rules(Rules) :-
	...
	(foreach(Rule, SortedRules),
	 foreach(ProcessedRule, ProcessedRules),
	 fromto(Clauses, [game_rule(Name, Arity, Head, Tail)|ClausesIn], ClausesIn, ProcessedRules) do
		(Rule=(Head:-Tail) -> ProcessedRule=(Headp:-Tailp) ; Rule=Head, Tail=true, ProcessedRule=Headp),
		functor(Head, Name, Arity),
		transform_formula(Head, Headp),
		transform_formula(Tail, Tailp)
	),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
:- mode legal_moves(++, ++, ?).
legal_moves(Role, State, Legals) :-
	setval(state, State),
	findall(Move, d_legal(Role, Move), Legals1),
	sort(Legals1, Legals).
...
goal_values(State, Values) :-
	setval(state, State),
	roles(Roles),
	(foreach(Role,Roles), foreach(Value,Values) do
		d_goal(Role, Value), !
	).
